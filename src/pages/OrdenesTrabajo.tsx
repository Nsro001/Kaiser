...
export default function OrdenesTrabajo() {
  ...
  useEffect(() => {
    ...
    const ordenesGuardadas = JSON.parse(localStorage.getItem('ordenes_trabajo') || '[]');
  ...
  const eliminarOrden = (id: string) => {
    ...
    localStorage.setItem('ordenes_trabajo', JSON.stringify(nuevasOrdenes));
  ...
  const cambiarEstado = (id: string, nuevoEstado: string) => {
    const nuevasOrdenes = ordenes.map(o => {
      if (o.id === id) {
        const ordenActualizada = { ...o, estado: nuevoEstado as OrdenTrabajo['estado'] };
        
        // Actualizar fechas según el estado
        if (nuevoEstado === 'en_proceso' && !o.fechaInicio) {
          ordenActualizada.fechaInicio = new Date().toISOString().split('T')[0];
        }
        if (nuevoEstado === 'completada' && !o.fechaFinalizacion) {
          ordenActualizada.fechaFinalizacion = new Date().toISOString().split('T')[0];
        }
  ...
  return (
    <div className="p-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Lista de Órdenes de Trabajo ({ordenesFiltradas.length})</CardTitle>
        </CardHeader>
        <CardContent>
            ...
            <div className="text-center py-8">
              ...
              <div className="text-gray-500">
                ...
                <div className="text-sm">
                  {ordenes.length === 0 
                    ? 'Las órdenes de trabajo se crean desde las cotizaciones'
              ...
              <TableBody>
                {ordenesFiltradas.map((orden) => (
                  <TableRow key={orden.id}>
                    ...
                    <TableCell>
                      <div className="text-sm">
                        <div>Creada: {new Date(orden.fechaCreacion).toLocaleDateString('es-CL')}</div>
                        {orden.fechaInicio && (
                          <div className="text-blue-600">Iniciada: {new Date(orden.fechaInicio).toLocaleDateString('es-CL')}</div>
                        )}
                        {orden.fechaFinalizacion && (
                          <div className="text-green-600">Finalizada: {new Date(orden.fechaFinalizacion).toLocaleDateString('es-CL')}</div>
                        )}
                      </div>
                    </TableCell>
      ...
      <Dialog open={isPreviewOpen} onOpenChange={setIsPreviewOpen}>
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Vista Previa - Orden de Trabajo</DialogTitle>
          </DialogHeader>
          {ordenSeleccionada && (
            <div className="space-y-6 p-6 bg-white">
              <div className="text-center border-b pb-4">
                <h2 className="text-2xl font-bold">ORDEN DE TRABAJO</h2>
                <p className="text-lg">N° {ordenSeleccionada.numero}</p>
                <p className="text-sm text-gray-600">
                  Fecha de creación: {new Date(ordenSeleccionada.fechaCreacion).toLocaleDateString('es-CL')}
                </p>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-gray-50 rounded-lg">
                  <div>
                    ...
                    <div>{new Date(ordenSeleccionada.fechaCreacion).toLocaleDateString('es-CL')}</div>
                  </div>
                  <div>
                    <div className="text-sm font-medium text-gray-600">Fecha de Inicio</div>
                    <div>{ordenSeleccionada.fechaInicio 
                      ? new Date(ordenSeleccionada.fechaInicio).toLocaleDateString('es-CL')
                      : 'No iniciada'
                    }</div>
                  </div>
                  <div>
                    <div className="text-sm font-medium text-gray-600">Fecha de Finalización</div>
                    <div>{ordenSeleccionada.fechaFinalizacion 
                      ? new Date(ordenSeleccionada.fechaFinalizacion).toLocaleDateString('es-CL')
                      : 'No finalizada'
                    }</div>
                  </div>
                </div>
              ...
              <div>
                <h3 className="font-semibold mb-2">Detalle del Trabajo:</h3>
                <Table>
                  <TableHeader>
                    <TableRow>
